...
class PaginatorContext(object):
    ...
    def sorted(self, objects, request, session_prefs=None):
        if  request.path_info == "/" and request.GET.get(self.SORT, None) is None: # used in main index page
            sort = 'active' #default index sort is active
        else:
...
def _paginated(request, objects, context):
    ...
    if total_pages > 1:
        def page_nums():
            ...
            if request.path_info == "/":
                url_builder = lambda n: mark_safe("%s?%s=%s" % ("/",context.PAGE,n))
            elif sort:
