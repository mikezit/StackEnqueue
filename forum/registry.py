...
from forum.templatetags.extra_tags import get_score_badge,get_reputation,gravatar
...
ui.register(ui.HEADER_LINKS,
            ...
            ui.Link(_('blog'), ui.Url(settings.BLOG_URL), weight=300, name='BLOG'),
            ui.Link(
            ...
            ui.Link(
                    ...
                    text=lambda u, c: "%s%s" % (gravatar(u,20),smart_unicode(u.username)),
                    url=lambda u, c: u.get_profile_url(),
                    post_code=lambda u, c: get_reputation(u)+get_score_badge(u),
...
ui.register(ui.USER_MENU,
            ...
            ui.UserMenuItem(
                label=_("flair"),
                url=lambda u, c: reverse('user_flair', kwargs={'name':c['user'].username,'id': c['user'].id}),
                span_attrs={'class': 'user-flair'},
                weight=100,
                name='AUTH_SETTINGS'
            ),
            ui.UserMenuItem(
