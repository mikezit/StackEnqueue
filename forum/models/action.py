...
class Action(BaseModel):
    ...
    tag = models.ForeignKey('Tag', null=True, related_name="actions")
...
class ActionProxy(Action):
    ...
    def describe_node(self, viewer, node):
        ...
        if node.parent:
            node_desc = _("%(link)s") % {'link': node_link}
            node_desc = SafeUnicode(node_desc)
        else:
            node_desc = node_link

        if viewer == node.author:
            return _("%(node_desc)s") % {
                'node_desc': node_desc,
                }

        user_node = self.hyperlink(node.author.get_profile_url(), self.friendly_ownername(viewer, node.author))
        ...
        return  _("%(node_desc)s") % {'node_desc': node_desc,}
