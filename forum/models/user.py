...
class User(BaseModel, DjangoUser):
    ...
    only_interesting_tag = models.BooleanField(default=False)
    ...
    weibo_gravatar = models.CharField(max_length=200, blank=True)
    use_weibo_gravatar = models.BooleanField(default=False)
    local_gravatar = models.CharField(max_length=200, blank=True)
    use_local_gravatar = models.BooleanField(default=False)
    ...
    def is_new_user(self):
        return self.reputation <= int(settings.REP_IS_NEW_USER)
    ...
    @true_if_is_super_or_staff
    def can_edit_tag(self, tag):
        return self == tag.author or self.reputation >= int(settings.REP_TO_EDIT_OTHERS ) 

    @true_if_is_super_or_staff
    def can_edit_post(self, post):
        return self == post.author or self.reputation >= int(settings.REP_TO_EDIT_OTHERS ) or ( post.nis.wiki and self.reputation >= int( settings.REP_TO_EDIT_WIKI))
...
class AuthKeyUserAssociation(models.Model):
    ...
    weibo_uid = models.CharField(max_length=64,null=True, blank=True)
    weibo_remind_in = models.CharField(max_length=64,null=True, blank=True)
    weibo_expires_at =  models.CharField(max_length=64,null=True, blank=True)
