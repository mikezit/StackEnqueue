...
class NodeContent(models.Model):
    ...
    def rendered(self, content):
        return auto_user_link(self, self._as_markdown(content, *['auto_linker','code_show']))
...
class Node(BaseModel, NodeContent):
    ...
    suggested        = models.ForeignKey('Action', null=True, unique=True, related_name="suggest_node")
    ...
    def pendding_suggestion(self):
        s_node = self.revisions.filter(suggested=True).order_by('-revised_at')
    ...
    def _create_revision(self, user, number,suggested=False, **kwargs):
        revision = NodeRevision(author=user, revision=number, node=self,suggested=suggested, **kwargs)
    ...
    def create_suggestion(self,user,**kwargs):
        ...
        if not self.pendding_suggestion :
            ...
            suggestion.body = kwargs["body"]
            
            if len(suggestion.title) !=0:
                suggestion.title = kwargs["title"]
                suggestion.tagnames = kwargs["tagnames"]
    ...
    def activate_revision(self, user, revision):
        ...
        if revision.suggested:
            ...
            self.update_last_activity(revision.author,time=revision.revised_at)
        else:
