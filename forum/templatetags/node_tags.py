...
def post_control(text, url,command=False, withprompt=False, confirm=False, title="",id=None):
    classes = (command and "ajax-command" or " ") + (withprompt and " withprompt" or " ") + (confirm and " confirm" or " ")
    return {'text': text, 'url': url, 'classes': classes, 'title': title,'id':id}
...
def post_controls(post, user):
    ...
    if post_type == "answer":
        ...
        controls.append(post_control(_('permanent link'), reverse('permanent_link', kwargs={'id' : post.id,"node_type":"answer"}),
        ...
        controls.append(post_control(_('permanent link'), reverse('permanent_link', kwargs={'id' : post.id,"node_type":"question"}),
                                     title=_("answer permanent link"), command=True, withprompt=True))
    else:
        pass
    ...
    if user.is_authenticated():
        try:
            ...
            #if user.can_edit_post(post):
            if not post.pendding_suggestion or post.pendding_suggestion and post.pendding_suggestion.author == user:
                controls.append(post_control(_('edit'), edit_url))
            elif user.can_edit_post(post): # post have suggestion 
                id = "edit-pending-%d" % post.id
                controls.append(post_control(_('edit')+'(0)',"#",id=id))
            else:
                pass
        except:
            pass

        if post_type == 'question' and user.can_retag_questions() and not user.can_edit_post(post):
            controls.append(post_control(_('retag'),reverse('retag_question', kwargs={'id': post.id}) ))
        ...
        if user.can_edit_post(post) or user.can_retag_questions():
...
def comments(post, user):
    ...
    for c in all_comments:
        ...
        context['by_owner'] = c.user == c.parent.user
        context['user'] = c.user
...
@register.inclusion_tag("node/last_edit_contributors_info.html")
def last_edit_contributors_info(node, verb=None):
    return {
        'node_verb': verb and verb or ((node.node_type == "question") and _("asked") or ( (node.node_type == "answer") and _("answered") or _("posted"))),
        'node': node,
    }

@register.inclusion_tag("node/contributors_info.html")
def suggest_author_info(node):
    return contributors_info(node,_("proposed"),True)

@register.inclusion_tag("node/contributors_info.html")
def tag_contributors_info(tag):
    added_time = tag.created_at
    author = tag.author
...
@register.inclusion_tag("node/contributors_info.html")
def contributors_info(node, verb=None,suggest_info=False):
    if suggest_info:
        added_time = node.pendding_suggestion.revised_at
        author = node.pendding_suggestion.author
    else:
        added_time = node.added_at
        author = node.author
...
@register.inclusion_tag("node/author_info.html")
def author_info(author, verb=None):
    return {
        'author': author,
    }
