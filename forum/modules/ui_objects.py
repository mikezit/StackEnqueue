...
class Url(object):
    ...
    def __call__(self, u, c):
        if not self.url_pattern.startswith("http://"):
...
class AjaxListGroup(AjaxMenuGroup):
    def render(self, context):
        return html.buildtag('a', self.label, **{'class': 'separator'}) + "".join([
            item.render(context) for item in self if item.can_render(context)
        ])

class AjaxLinkItem(ObjectBase):
    def __init__(self, label, url, a_attrs=None, span_label='',pre_code='', post_code='', span_attrs=None, visibility=None, weight=500, name=''):
        super(AjaxLinkItem, self).__init__(visibility, weight, name)
        self.label = self.Argument(label)
        self.url = self.Argument(url)
        self.a_attrs = self.Argument(a_attrs or {})
        self.pre_code = self.Argument(pre_code)
...
class UserMenuItem(AjaxLinkItem):
