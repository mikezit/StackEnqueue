...
def get_suggested_posts():
    return Node.objects.filter(suggested__isnull=False)
    #return Action.objects.filter(canceled=False, action_type="suggest").order_by('-action_date')[0:30]
...
#todo: this is the quick and dirty way of implementing a bootstrap mode
    ...
    settings.REP_GAIN_BY_SUGGESTION.set_value(15)
...
def suggested_posts(request):
    return ('osqaadmin/suggested_posts.html', {
    'suggested_posts': get_suggested_posts(),
    })
