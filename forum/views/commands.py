...
class NotEnoughRepPointsException(CommandException):
    def __init__(self, privilege):
        html_text = ""
        if type(privilege) == str or type(privilege) == unicode:
            html_text = _(
                        """Sorry, but you don't have enough reputation points to %(action)s.<br />Please check the <a href='%(faq_url)s'>faq</a>"""
                        ) % {'action': privilege, 'faq_url': reverse('faq')}
        else:
            html_text = _("""<a href='%(action_url)s'>%(action)s</a> requires %(reputation)s <a href='%(faq_url)s'>reputations</a>""") %  {
                'action':_(privilege.field_context["label"]),
                'reputation':privilege.field_context["reputation"].value,
                'faq_url': reverse('faq'),
                'action_url':"/privileges/"+privilege.field_context["label"].replace(" ","-")
                }

        super(NotEnoughRepPointsException, self).__init__(html_text)
...
class AnonymousNotAllowedException(CommandException):
    def __init__(self, action):
        super(AnonymousNotAllowedException, self).__init__(
            _(
                """Sorry but anonymous users cannot %(action)s.<br />Please <a href='%(login_url)'>login</a> or create an account <a href='%(signin_url)s'>here</a>."""
                ) % {
                'action': action,
                'signin_url': '/account/local/register/',
                'login_url': reverse('auth_signin')
                }
            )
...
def vote_post(request, id, vote_type):
    ...
    if not (vote_type == 'up' and user.can_vote_up() or user.can_vote_down()):
        if vote_type == 'up':
            raise NotEnoughRepPointsException(VOTE_UP_PAGE)
        else:
            raise NotEnoughRepPointsException(VOTE_DOWN_PAGE)
    ...
    if int(settings.START_WARN_VOTES_LEFT) >= votes_left:
        #response['message'] = _("You have %(nvotes)s %(tvotes)s left today.") % {'nvotes': votes_left, 'tvotes': ungettext('vote', 'votes', votes_left)}
        response['message'] = _("You have %(nvotes)s votes left today.") % {'nvotes': votes_left }
...
def flag_post(request, id):
    ...
    if not (user.can_flag_offensive(post)):
        raise NotEnoughRepPointsException(FLAG_POSTS_PAGE)
...
def comment(request, id):
    ...
    if 'id' in request.POST:
        ...
        if not user.can_comment(post):
            raise NotEnoughRepPointsException(COMMENT_EVERYWHERE_PAGE)
...
def wikify(request, id):
    ...
    if node.nis.wiki:
        ...
        if not user.can_wikify(node):
            raise NotEnoughRepPointsException(CREATE_WIKI_POSTS_PAGE)
...
def subscribe_tag(request,tag):
    ...
    if len(ts) != 0: 
        t = ts[0]
        if t.subscribed : #unsubscribe node
            if t.reason != 'none':
                ts.update(subscribed=False)
            else: #not used , delete
                ts.delete()
                has_subscribed = False
        else:
            ts.update(subscribed=True) #subscribe old node
    else: #subscribe new node
        try:
            t = Tag.objects.get(name=tag)
            mt = MarkedTag(user=request.user,tag=t,subscribed=True)
            mt.save()
            has_subscribed = True
        except:
            raise Http404()
    ...
    return {
        'commands': {
            'set_subscription_tag_button': [tag , has_subscribed and _('unsubscribe1') or _('subscribe1')]
        }
    }
...
def matching_tags(request):
    ...
    possible_tags = Tag.active.filter(pinyin__icontains = request.GET['q'])
    tag_output = ''
...
def related_questions(request):
    if request.POST and request.POST.get('title', None):
        ...
        can_rank=False
        questions = Question.objects.filter(title__icontains=keywords)
...
@decorate.withfn(command)
def permanent_link(request, id , node_type):
    ...
    url = '%s%s' % (settings.APP_BASE_URL, post.get_absolute_url())
