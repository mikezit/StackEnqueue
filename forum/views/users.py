...
from forum.actions import EditProfileAction, FavoriteAction, BonusRepAction, SuspendAction,ProfileViewAction
...
class UserListPaginatorContext(pagination.PaginatorContext):
    def __init__(self):
        super (UserListPaginatorContext, self).__init__('USERS_LIST', sort_methods=(
            ('reputation', UserReputationSort(_('reputation'), '-reputation', _("sorted by reputation"))),
            ('newest', pagination.SimpleSort(_('recent'), '-date_joined', _("newest members"))),
            ('last', pagination.SimpleSort(_('oldest'), 'date_joined', _("oldest members"))),
            ('name', pagination.SimpleSort(_('by username'), 'username', _("sorted by username"))),
        ), pagesizes=(20, 36, 60))
...
def crop_gravatar(request,user_id,original_id):
    ...
    if request.method == "POST":
        form = CroppedForm(request.POST, initial={'original': original})
        if form.is_valid():
            cropped = form.save(commit=False)
            cropped.save()
            user.local_gravatar = os.path.basename(cropped.image.path)
            user.save()
            return HttpResponseRedirect(reverse("edit_gravatar",args=[user.id]))


    form = CroppedForm(None, initial={'original': original})

    return render_to_response( 'crop_gravatar.html', {
            'user'     : user,
            'form'     : form,
            'cropped'  : cropped,
            'original' : original,
...
def edit_gravatar(request,id):
    user = get_object_or_404(User, id=id)

    if not (request.user.is_superuser or request.user == user):
        return HttpResponseUnauthorized(request)

    upform = OriginalForm(request.POST or None, request.FILES or None)

    if request.method == "POST":
        if not request.FILES:
            gform = GravatarSelectForm(user,request.POST)
            if gform.is_valid():
                if request.POST.get('local_gravatar', False) == 'on':
                    user.use_local_gravatar = True
                    user.use_weibo_gravatar = False
                elif request.POST.get('weibo_gravatar', False) == 'on':
                    user.use_weibo_gravatar = True
                    user.use_local_gravatar = False
                else:
                    user.use_weibo_gravatar = False
                    user.use_local_gravatar = False
                user.save()
        ...
        if upform.is_valid():
            original = upform.save()
            return HttpResponseRedirect(reverse("crop_gravatar",args=[user.id,original.id]))

    gravatar_form = GravatarSelectForm(user)
    return render_to_response('select_gravatar.html', {
            'gravatar_form':gravatar_form,
            'form':upform,
            'user': user,
            }, context_instance=RequestContext(request))
...
def edit_user(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user.display_name = sanitize_html(form.cleaned_data['display_name'])
...
def user_profile(request, user):
    ...
    user_tags = Tag.objects.filter(Q(nodes__author=user) | Q(nodes__children__author=user)).annotate(user_tag_usage_count=Count('name')).order_by('-user_tag_usage_count')
    ...
    update_profile_view_times(request,user)
...
def change_interestingTag_viewMode(request, id):
    user = get_object_or_404(User, id=id)
    ...
    if request.method == 'POST':
       if request.POST.get('tagmode') == "true":
           user.only_interesting_tag = True
       else:
           user.only_interesting_tag = False
       user.save()

    return HttpResponse(simplejson.dumps("ok"), mimetype="application/json")

@user_view('users/tags.html', 'tags', _('follow'), _('tags'))
def user_tags(request, user):
    tags = MarkedTag.objects.filter(user=user).filter(subscribed=True).values_list('tag__name', flat=True)
    vistor_tags = []
    if request.user.is_authenticated():
        vistor_tags = MarkedTag.objects.filter(user=request.user).filter(subscribed=True).values_list('tag__name', flat=True)    
    return {"vistor":request.user,"view_user" : user,"tags" : tags,"vistor_tags":vistor_tags}
...
def user_recent(request, user):
    activities = user.actions.exclude(
            action_type__in=("voteup", "votedown", "voteupcomment", "flag", "newpage", "editpage","awardpoints","bonusrep","favorite")).order_by('-action_date')[:USERS_PAGE_SIZE]
...
def user_reputation(request, user):
    ...
    reps = user.reputes.filter(action__canceled=False).order_by('-date')
    date_rep={}
    for rep in reps:
        rep_date = datetime.datetime(rep.date.year,rep.date.month,rep.date.day)
        if rep_date not in date_rep.keys():
            date_rep[rep_date]=rep.positive + rep.negative
        else:
            date_rep[rep_date] +=rep.positive + rep.negative            

    date_rep = sorted(date_rep.items(),reverse=True)
    
    rep_by_date = lambda d:user.reputes.filter(Q(date__year=d.year)&Q(date__month=d.month)&Q(date__day=d.day)).order_by('-date')

    for i in range(3):
        if len(date_rep) > i:
            date_rep[i]=date_rep[i]+(rep_by_date(date_rep[i][0]),)

    return {"view_user": user, "reputation": date_rep, "graph_data": graph_data}

@user_view('users/votes.html', 'votes', _('give votes'), _('user vote record'),tabbed=False)
...
def update_profile_view_times(request,user):
    last_seen_in_profile = request.session.get('last_seen_in_profile', {})
    last_seen = last_seen_in_profile.get(user.id, None)

    if (not last_seen) or (last_seen < user.last_activity):
        ProfileViewAction(user, ip=request.META['REMOTE_ADDR']).save()
        last_seen_in_profile[user.id] = datetime.datetime.now()
        request.session['last_seen_in_profile'] = last_seen_in_profile
