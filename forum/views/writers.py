...
from forum.actions import AskAction, AnswerAction, ReviseAction,SuggestAction, TagReviseAction,RollbackAction, RetagAction, AnswerToQuestionAction,SuggestAcceptAction,SuggestRejectAction,TagSuggestAction,TagSuggestRejectAction,TagSuggestAcceptAction
...
def ask(request):
    ...
    if request.user.is_anonymous() or request.user.is_new_user():
        if not request.GET:
            return render_to_response('ask_tips.html', {
...
def edit_question(request, id):
    ...
    if request.user.can_edit_post(question):
        ...
        return _edit_question(request, question,action_class=SuggestAction)
...
def retag_question(request, id):
    ...
    if not request.user.can_retag_questions():
        raise Http404
...
def edit_tag(request,id):
    ...
    form = None
    action_class=TagSuggestAction
    
    if request.user.can_edit_tag(tag):
        action_class=TagReviseAction

    if request.POST:
        form = EditTagForm(tag,request.user,None,request.POST)
        if form.is_valid():
            if form.has_changed():
                if request.GET.get("improve") == "true":
                    suggest =  tag.pendding_suggestion
                    if suggest:
                        TagSuggestAcceptAction(user=request.user, tag=tag, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggest))

                if request.user.is_superuser:
                    sponsor_tag_img =  form.data["sponsor_tag_img"] 
                    if tag.sponsor_tag_img != sponsor_tag_img and request.user.is_superuser:
                        tag.sponsor_tag_img = sponsor_tag_img
                        tag.save()

                action = action_class(user=request.user, tag=tag, ip=request.META['REMOTE_ADDR']).save(data=form.cleaned_data)
                return HttpResponseRedirect("/tags/%s" % tag.name)
    else:
        form = EditTagForm(tag,request.user )

    return render_to_response('edit_tag.html', {
        'form' : form,
        'tag' : tag,
        'tab' : 'ask',
        'is_suggestion':action_class == TagSuggestAction,
        }, context_instance=RequestContext(request))
...
def _edit_question(request, question, template='question_edit.html', summary='', action_class=ReviseAction, allow_rollback=True, url_getter=lambda q: q.get_absolute_url()):
    if request.method == 'POST':
        ...
        if not 'select_revision' in request.POST and form.is_valid():
            if form.has_changed():
                if request.GET.get("improve") == "true":
                    suggest =  question.pendding_suggestion
                    if suggest:
                        SuggestAcceptAction(user=request.user, node=question, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggest))
        ...
        form = EditQuestionForm(question, request.user,revision=revision, initial={'summary': summary})

    return render_to_response(template, {
        'question': question,
        'revision_form': revision_form,
        'form' : form,
        'is_suggestion':action_class == SuggestAction,
    }, context_instance=RequestContext(request))
...
def edit_answer(request, id):
    ...
    if request.method == "POST":
        ...
        if not 'select_revision' in request.POST and form.is_valid():
            if form.has_changed():
                if request.GET.get("improve") == "true":
                    SuggestAcceptAction(user=request.user, node=answer, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=question.pendding_suggestion))
                action = action_class(user=request.user, node=answer, ip=request.META['REMOTE_ADDR']).save(data=form.cleaned_data)
        ...
        form = EditAnswerForm(answer, request.user,revision=revision)
...
def tag_suggested_edits_action(request,id,reason):
    return suggested_edits_action(request,id,reason,class_type="tag")

def suggested_edits_action(request,id,reason,class_type="node"):
    if not request.user.is_authenticated():
        raise Http404

    if class_type is "tag":
    ...
    if suggestion is None:
        ret_json["MiniHtml"]="<h3>error</h3>"
        ret_json["Message"]=_("suggestion has been handled by others!")

    elif request.user.can_edit_post(post):
        ret_json["RedirectTo"]=post.get_absolute_url()

        if reason == "approve":
            ret_json["Success"]=True
            if class_type is "tag":
                TagSuggestAcceptAction(user=request.user, tag=post, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggestion))
            else:
                SuggestAcceptAction(user=request.user, node=post, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggestion))

        elif reason == "reject":
            ret_json["Success"]=True
            if class_type is "tag":
                TagSuggestRejectAction(user=request.user, tag=post, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggestion))
            else:
                SuggestRejectAction(user=request.user, node=post, ip=request.META['REMOTE_ADDR']).save(data=dict(suggest=suggestion))

        else:
            ret_json["MiniHtml"]=""
            ret_json["Message"]=_("error request!")

    else:
        ret_json["MiniHtml"]=""
        ret_json["Message"]=_("you have no privilege to edit this post!")
