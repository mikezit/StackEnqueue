...
from forum.utils.weibo_action import weibo_post ,weibo_get
...
class AwardAction(ActionProxy):
    ...
    def target(self,viewer=None):
        return _("<a class='medal'><span class='badege%(badge_type)s'>&#9679</span>&nbsp;%(badge_name)s</a>") % {
            'badge_name':self.award.badge.name,
            'badge_type':self.award.badge.type
            }
...
class SuspendAction(ActionProxy):
    ...
    def target(self,viewer=None):
        return self.affected_links(viewer)
