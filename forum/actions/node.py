...
from forum.utils.weibo_action import weibo_post
import urlparse
import logging
...
class AskAction(NodeEditAction):
    ...
    def process_action(self):
        weibo_post(self.user,_("i have ask a question at %(app_name)s:%(question_title)s %(question_url)s") % {
               'app_name':settings.APP_SHORT_NAME,
               'question_title':self.node.title,
               'question_url':urlparse.urljoin(settings.APP_URL,self.node.get_absolute_url()),
        })

    def describe(self, viewer=None):
        return _("%(user)s asked %(question)s") % {
            'user': self.hyperlink(self.user.get_profile_url(), self.friendly_username(viewer, smart_unicode(self.user))),
            'question': self.hyperlink(self.node.get_absolute_url(), smart_unicode(self.node.title))
        }

    def target(self,viewer=None):
        return self.hyperlink(self.node.get_absolute_url(), smart_unicode(self.node.title))
...
class AnswerAction(NodeEditAction):
    ...
    def target(self,viewer=None):
        question = self.node.parent        
        return self.hyperlink(self.node.get_absolute_url(), question.title)
...
class CommentAction(ActionProxy):
    ...
    def target(self,viewer=None):
        return self.describe_node(viewer, self.node.parent)
...
class ReviseAction(NodeEditAction):
    ...
    def target(self,viewer=None):
        return self.describe_node(viewer, self.node)

    def get_absolute_url(self):
        return self.node.get_revisions_url()

class SuggestAcceptAction(NodeEditAction):
    verb = _("accept suggestion")

    def repute_users(self):
        self.repute(self.node.last_suggest.by, int(REP_GAIN_BY_SUGGESTION))

    def process_data(self, suggest=None):
        previous = self.node.active_revision
        self.node.activate_revision(self.user, suggest)
        self.extra = "%d:%d" % (previous.revision, suggest.revision)

    def process_action(self):
        self.node.last_edited = self.node.last_suggest
        self.node.save()
...
class SuggestRejectAction(NodeEditAction):
    def process_data(self, suggest=None):
        suggest.delete()
        #previous = self.node.active_revision
        #self.node.activate_revision(self.user, suggest)
...
class SuggestAction(NodeEditAction):
    verb = _("suggested")

    def process_data(self, **data):
        revision_data = self.create_revision_data(**data)
        revision = self.node.create_suggestion(self.user, **revision_data)
        self.extra = revision.revision
        self.summary = revision.summary

    def process_action(self):
        self.node.suggested = self
        self.node.save()
...
class AnswerToCommentAction(ActionProxy):
    ...
    def target(self,viewer=None):
        return self.describe_node(viewer, self.node.abs_parent)
