...
from forum.settings import THINGS_SUMMARY_LENGTH
...
class VoteAction(ActionProxy):
    ...
    def target_vote(self, vote_desc, viewer=None):
...
class VoteUpAction(VoteAction):
    ...
    def target(self, viewer=None):
        return self.target_vote(_("voted up"), viewer)
...
class VoteDownAction(VoteAction):
    ...
    def target(self, viewer=None):
        return self.target_vote(_("voted down"), viewer)
...
class VoteUpCommentAction(VoteUpAction):
    ...
    def target(self, viewer=None):
        return self.target_vote(_("liked"), viewer)
...
class FlagAction(ActionProxy):
    ...
    def target(self,viewer=None):
        return "%(post_desc)s" % self.describe_node(viewer, self.node)
...
class AcceptAnswerAction(ActionProxy):
    ...
    def target(self, viewer=None):
        answer = self.node
        question = answer.parent
        return self.hyperlink(question.get_absolute_url(), question.title)
...
class ProfileViewAction(DummyActionProxy):
    def __init__(self, user, ip=None):
        self.user = user
        super(ProfileViewAction, self).__init__(ip)
        self.__class__.hooks=[]
        
    def process_action(self):
        self.user.profile_views = F('profile_views') + 1
        self.user.save()

class TagViewAction(DummyActionProxy):
    def __init__(self, tag, ip=None):
        self.tag = tag
        super(TagViewAction, self).__init__(ip)
        self.__class__.hooks=[]
        
    def process_action(self):
        self.tag.view_times = F('view_times') + 1
        self.tag.save()
